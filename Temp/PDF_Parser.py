...
def extract_metadata_from_block(block):
    ...
    for idx, line in enumerate(lines):
        ...
        if lower.startswith("reply-to:") and not metadata["Reply-To"]:
            ...
            if not re.match(r"\b(?:mon|tue|wed|thu|fri|sat|sun)[a-z]*[,]?\s+\w+\s+\d{1,2}[,]?\s+\d{4}", subject, re.IGNORECASE):
        ...
        if idx < 6:
            ...
            if not metadata["Date"] and re.search(r"\b\d{1,2}:\d{2}\s*(AM|PM|am|pm)", line):
        ...
        if not started:
            if any(lower.startswith(k) for k in header_keywords):
                ...
                if skip_header_lines <= max_header_lines:
                    continue
            if len(line_clean) > 30 and not any(x in lower for x in ["gmail", "outlook", "unsubscribe", "@", "http"]):
                started = True
            ...
            if re.match(r"--- Page \d+ ---", line_clean):
                continue
            if "unsubscribe" in lower or "view in browser" in lower or "gmail - " in lower:
                continue
